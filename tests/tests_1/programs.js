...
exports.runTests = function(filter)  {
   ...
   function test(name, start, text, query, check)  {
      ...
      util.addTest();
      curTest = name;
      if (typeof text == "number") text = file.slice(start, text)      query.file = "#0";
      var fdata =  {
         type:"part", 
         name:"file", 
         text:text, 
...
;
      server.request( {
         ...
         function(err, data)  {
            try {
               if (err) fail(err)                else check(data)            }
            catch (e) {
...
;
  ...
  file",  {
   ...
   test("long_line", 6141, 6774,  {
         type:"type", // C1 SRC, C2 DST
         end:603      }, 
      function(data)  {
         eq(data.type, "bool");
      }
   );
   test("long_line_replace", 6141, "var x = 'hi'; x;",  {
         type:"type", // T3 DST, C2 SRC
         end:15      }, 
      function(data)  {
         eq(data.type, "string");
      }
